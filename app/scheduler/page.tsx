...
const Scheduler = ({propertyInfo, onClose=()=>{}, setTitle, setSubtitle, setStep, onTimeSelect }) => {
  const [selectedScheduleDate, setSelectedScheduleDate] = useState(new Date());
  console.log("selectedScheduleDate", selectedScheduleDate)
  let defaulthours = [
    '09:00 AM',
    '10:00 AM',
    '11:00 AM',
          ...
          {availablepropertyhours?.map((item, idx) => (
            <div
              onClick={() => {
                ...
                //setSubtitle("July 31st, 2024 at 10:00am")
              }}
              key={idx}
...
const ThankYouScreen = ({ onClose }) => (
  <div className="p-6 text-center bg-white rounded-lg max-w-md mx-auto">
    ...
    {/* <button
      onClick={onClose}
      className="border-none justify-center	w-full py-3 bg-green-400 text-white font-semibold rounded hover:bg-green-500 transition-colors duration-300"
    >
      Close
    </button> */}
  </div>
...
const Popup = ({propertyInfo, uuid, isOpen, onClose, integrationDetails }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="border-ll-text/10 w-[calc(100%-10px)] h-auto sm:w-[465px] sm:h-[759px] mx-auto relative top-1/2 flex -translate-y-1/2 flex-row flex-wrap overflow-hidden rounded-[36px] bg-white fade-in">
        <div className="justify-start absolute top-0 w-full z-10 h-20 flex flex-row bg-white px-5 sm:px-6 border-b border-ll-borders/10">
          <button
            onClick={() => {
              if (step > 1) {
                if(step == 3) {
          ...
          <button
            ...
            aria-label="Close the tour scheduler"
            onClick={onClose}
          >
            <CloseIcon />
        ...
        <div className="h-view-height mt-20 overflow-auto w-full relative styled-scroll flex-grow flex flex-col content-start">
          {step === 1 && (
            <ul className="px-8 flex-grow-0">
          ...
          {step === 2 && <Scheduler propertyInfo={propertyInfo} setTitle={setTitle} setSubtitle={setSubtitle} setStep={setStep} onClose={() => setIsCalendarOpen(false)} onTimeSelect={handleTimeSelect} />}
          {step === 3 && <Confirmation setStep={setStep} setTitle={setTitle} setSubtitle={setSubtitle} scheduleDate={scheduleDetails.date} scheduleTime={scheduleDetails.time} onClose={onClose} onSubmit={onSubmit} />}
          {step === 4 && <ThankYouScreen onClose={onClose} />}
          <a href="https://leaseleads.co" target="_blank" rel="noopener" aria-label="link opens in a new tab" className="relative mx-auto mt-auto mb-4 block h-6 w-auto justify-center self-end">
            <img
              className='h-6'
              src="https://imagedelivery.net/d3WSibrZmE8m_HEZW60OcQ/f1241207-f4a8-4e8a-030e-110332ad6200/public"
            />
            <span style={{ display: 'none' }}>LeaseLeads</span>
          </a>
        </div>
      </div>
    </div>
...
export const StickyButton = ({ onClick }) => {
  return (
    <div
      style={{ left: '-80px' }}
      className="fixed top-1/2 transform -translate-y-1/2 z-40 h-full flex items-center">
      <button
        onClick={onClick}
        className="bg-[#f09f54] text-[#015057] text-sm sm:text-lg font-semibold uppercase tracking-wide 
        leading-none py-2 px-4  border-[#f09f54] transform rotate-[-90deg] origin-left-top hover:bg-[#e58d3b]
        transition ease-out w-full"
...
const App = ({ integrationDetails, uuid }) => {
  ...
  useEffect(() => {
    setpropertyInfo(async () => {
      return await getProperty(integrationDetails["api-entrata"])
    })
  },[])

  return (
    <div>
      <StickyButton onClick={togglePopup} />
      <Popup propertyInfo={propertyInfo} isOpen={isPopupOpen} onClose={togglePopup} uuid={uuid} integrationDetails={integrationDetails} />
    </div>
  );
};
